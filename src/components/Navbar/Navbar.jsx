    ...
    return (
      <div className="relative" onClick={(e) => e.stopPropagation()}>
        ...
        >
          ...
          <span className="hidden md:block text-amber-100 text-sm font-cinzel">
            {userInfo?.name}
          </span>
        </button>
        ...
        <div className={`absolute right-0 top-full pt-4 w-56 transition-all duration-300 transform 
          ...
          <div className="bg-[#1a120b] border border-amber-900/50 rounded-lg shadow-2xl 
  ...
  return (
    <>
      <nav className={`fixed top-0 left-0 w-full z-50 transition-all duration-500 border-b border-transparent 
        ${scrolled ? 'bg-[#1a120b]/90 backdrop-blur-md py-3 shadow-lg border-amber-900/30' : 'bg-transparent py-6'}`}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex justify-between items-center">
          ...
          <div className="hidden lg:flex items-center gap-8">
            {navLinks.map((link) => (
              <NavLink key={link.name}
                to={link.href}
                className={({ isActive }) => `text-sm font-cinzel uppercase tracking-[0.15em] transition-all duration-300 relative group py-2 
          ...
          <div className="lg:hidden flex items-center gap-4">
            <Link to="/cart" className="relative text-amber-100 hover:text-amber-500">
              <FiShoppingCart size={20} />
              {totalItems > 0 && <span className="absolute -top-2 -right-2 bg-amber-600 text-white text-[9px] w-4 h-4 
              rounded-full flex items-center justify-center font-bold">{totalItems}</span>}
            </Link>
            <button onClick={() => setIsOpen(!isOpen)} className="text-amber-500 text-2xl">{isOpen ? <FiX /> : <FiMenu />}</button>
          </div>
        </div>

        <div className={`fixed inset-0 bg-[#1a120b] z-[100] flex flex-col items-center justify-center gap-8 transition-transform 
          duration-500 overflow-y-auto ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}>
          <button onClick={() => setIsOpen(false)} className="absolute top-6 right-6 text-amber-500 text-3xl"><FiX /></button>
          {navLinks.map((link) => (
            <NavLink key={link.name}
              to={link.href}
              onClick={() => setIsOpen(false)}
              className={({ isActive }) => `text-2xl font-dancingscript 
            ${isActive ? 'text-amber-500' : 'text-amber-100'}`}>{link.name}</NavLink>
          ))}
          <div className="h-px w-20 bg-amber-900/50 my-2"></div>
          {isAuthenticated ? (
            <div className="flex flex-col items-center gap-6">
              <Link to="/myorders"
                onClick={() => setIsOpen(false)}
                className="text-amber-100/80 hover:text-amber-500 flex items-center gap-3 text-xl"><FiPackage /> My Orders</Link>
              <Link to="/my-bookings"
                onClick={() => setIsOpen(false)}
                className="text-amber-100/80 hover:text-amber-500 flex items-center gap-3 text-xl"><FiCalendar /> My Bookings</Link>
              <button onClick={() => { handleLogout(); setIsOpen(false); }}
                className="text-red-400 flex items-center gap-3 text-xl mt-4"><FiLogOut /> Logout</button>
            </div>
          ) : (
            <button onClick={() => { navigate('/login'); setIsOpen(false); }}
              className="text-xl font-cinzel text-amber-500 border border-amber-500 px-8 py-2 rounded-full">Login</button>
          )}
        </div>
      </nav>
